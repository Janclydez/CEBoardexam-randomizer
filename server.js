...
const QUESTIONS_DIR = path.join(process.cwd(), 'psadquestions');
...
app.get('/tags', async (req, res) => {
  try {
    ...
    const subTagMap = {}; // { subTag: [mainTag, ...] }

    // read only q*.json (ignore images, faculty, etc.)
    const jsonFiles = files.filter(f => /^q\d+\.json$/i.test(f));
    ...
    for (const f of jsonFiles) {
      try {
        ...
        const main = (j.mainTag || '').trim(); // e.g., "PSAD - Structural Theory"
        const sub  = (j.subTag  || '').trim(); // e.g., "Slope and Deflection"
        if (!main || !sub) continue;

        mainSet.add(main);
        subSet.add(sub);

        if (!subTagMap[sub]) subTagMap[sub] = [];
        if (!subTagMap[sub].includes(main)) subTagMap[sub].push(main);
      } catch (e) {
        console.warn('Skipping bad file:', f, e.message);
      }
    }

    // sort for nice UI
    const mainTags = [...mainSet].sort((a,b) => a.localeCompare(b));
    const subTags  = [...subSet].sort((a,b) => a.localeCompare(b));
    Object.keys(subTagMap).forEach(k => subTagMap[k].sort((a,b) => a.localeCompare(b)));

    res.json({ mainTags, subTags, subTagMap });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Failed to build tags' });
  }
});
