...
# ================= PLACEHOLDER #1 (TOP): <style> ... </header> =================
# MUST start with <style> and MUST end with </header>
# IMPORTANT: Use HTML entities (&#...;) in the header HTML.
#            Use CSS unicode escapes for moon/sun (content:"\1F319" etc.)
$topReplacement = @'
<style>
    body {
...
.slider:before {
  content: "\1F319"; /* 🌙 crescent moon (default / light mode) */
  position: absolute;
  height: 22px;
  width: 22px;
  left: 3px;
  ...
  /* IMPORTANT: force emoji-capable font */
...
<body>
  <header>
    ...
    <div class="title-group">
      ...
      <a href="../surveying.html" class="back-btn-header">&#11013; Back to Subject Topics</a>
    </div>
  <nav id="dropdownMenu" aria-label="Site">
    <ul class="menu-list">
      ...
      <li class="dropdown">
        ...
        <ul class="submenu">
          <li><a href="/subjects/psad.html">&#128216; Structural Engineering (PSAD)</a></li>
          <li><a href="/subjects/hydraulics.html">&#128167; Hydraulics Engineering</a></li>
          <li><a href="/subjects/geotech.html">&#129704; Geotechnical Engineering</a></li>
          <li><a href="/subjects/mstc.html">&#9881;&#65039; MSTC (Math, Surveying, etc)</a></li>
        </ul>
  </nav>
...
# ================= PLACEHOLDER #2 (BOTTOM): toggleSolution .. before </body> ===
# MUST start with <script> and MUST end with </script>
# IMPORTANT: Use HTML entities for icons: 📚, 🧭, ×, etc.
$bottomReplacement = @'
<script>
...
">
  ...
  ">
    <strong style="color: #18398A;">&#129517; Jump to:</strong>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  overlay.addEventListener('dblclick', (e) => {
    if (!expandedImg.contains(e.target)) closeOverlay();
  });
...
# --- sanity checks ---
if ($topReplacement   -notmatch '(?is)^\s*<style\b')   { throw "TOP replacement must start with <style>" }
if ($topReplacement   -notmatch '(?is)</header>\s*$')  { throw "TOP replacement must end with </header>" }
if ($bottomReplacement -notmatch '(?is)^\s*<script\b')  { throw "BOTTOM replacement must start with <script>" }
if ($bottomReplacement -notmatch '(?is)</script>\s*$')  { throw "BOTTOM replacement must end with </script>" }

# --- patterns ---
$topPattern    = '(?is)<style\b[^>]*>.*?</header>'
$bottomPattern = '(?is)<script\b[^>]*>\s*function\s+toggleSolution\b.*?(?=</body>)'
...
$files = if ($Recurse) {
  Get-ChildItem -File -Recurse -Filter "*.html" | Where-Object { $_.Name -notmatch '\.bak' }
} else {
  Get-ChildItem -File -Filter "*.html" | Where-Object { $_.Name -notmatch '\.bak' }
}

if ($files.Count -eq 0) { Write-Host "No .html files found."; exit 0 }

# Write as UTF-8 WITH BOM (prevents Windows PS5.1 mojibake)
...
foreach ($f in $files) {
  ...
  if ($t -match $topPattern) {
    $t = [regex]::Replace($t, $topPattern, $topReplacement, 1)
  }

  if ($t -match $bottomPattern) {
    $t = [regex]::Replace($t, $bottomPattern, $bottomReplacement + "`r`n", 1)
  }

  if ($t -ne $orig) {
    Copy-Item -LiteralPath $p -Destination ($p + ".bak_bulkreplace") -Force
    [System.IO.File]::WriteAllText($p, $t, $utf8WithBom)
...
Write-Host "Backups: .bak_bulkreplace"
